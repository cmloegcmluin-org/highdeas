...
Working review app: capture → ingest → local transcription → a local web page where you play each memo, edit its transcript, name it, pick Notesnook or Drive, submit, or delete. Submitting a **Notesnook** memo creates a real note via the Inbox API; a **Drive/music** memo moves the recording into a dated `..._NOT_YET_PROCESSED_MUSIC` folder under `voice memos (top level)` with an accompanying `.docx`. Either way — and on delete — the recording is retired **out of the inbox**: Drive to the Drive folder, Notesnook/delete to a local `bin/` (recoverable), so the inbox only ever holds unprocessed recordings. Not yet wired: undo, the archive/bin **tab** to view and restore, and the multi-clip "one shared numbered doc" grouping.
